...
// Validation is driven by comments: `// @capnweb-validate` opts a class in,
// and `// @capnweb-validate-ignore` opts a method back out. These cover
// directive parsing and how the directives reach the emitted validators.
...
export class Api extends RpcTarget {
  ...
  it("leaves an annotated ambient `declare class` alone", () => {
    // A comment can be written on any declaration, but an ambient one has no
    // runtime prototype in this module to wrap.
    expect(
      maybeTransform(
        `// @capnweb-validate
...
export type { Api };`
  ...
  it("leaves an annotated class inside `declare module` alone", () => {
    expect(
      maybeTransform(
...
class Api extends RpcTarget {
  ...
  it("rejects the opt-in directive written on a member", () => {
    // `@capnweb-validate` opts in a whole class, so on a member it is a
    // mistake. Say so rather than silently doing nothing.
    expect(
      transformError(
        `// @capnweb-validate
class Api extends RpcTarget {
  ...
  greet(name: string): string {
    ...
    ).toContain("belongs on the class declaration");
  });
});
