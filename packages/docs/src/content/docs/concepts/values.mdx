...
A `Set` cannot contain a promise or `Blob` directly when sent over a connection. Stubs are valid set
elements, and promises and `Blob`s may still appear inside a plain object or array used as an
element.
